...
Sessions are stored in SQLite, so logins persist for up to 30 days (configurable via `SESSION_MAX_AGE_DAYS`) and across restarts as long as `DB_PATH` points to durable storage.
